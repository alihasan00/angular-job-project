                ...
                <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                    <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                        <div class="text-center mb-8">
                            ...
                            <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Welcome to Blog!</div>
                        ...
                        <form [formGroup]="loginForm" (ngSubmit)="onSubmit()">
                            ...
                            <input pInputText id="email1" type="text" placeholder="Email address" class="w-full md:w-[30rem] mb-2" formControlName="email" [class.ng-invalid]="loginForm.get('email')?.invalid && loginForm.get('email')?.touched" />
                            <div *ngIf="loginForm.get('email')?.invalid && loginForm.get('email')?.touched" class="text-red-500 text-sm mb-6">
                                <div *ngIf="loginForm.get('email')?.errors?.['required']">Email is required</div>
                                <div *ngIf="loginForm.get('email')?.errors?.['email']">Please enter a valid email address</div>
                            </div>
                            <div *ngIf="!loginForm.get('email')?.invalid || !loginForm.get('email')?.touched" class="mb-6"></div>
                            ...
                            <div *ngIf="loginForm.get('password')?.invalid && loginForm.get('password')?.touched" class="text-red-500 text-sm mb-2">Password is required</div>
                            <div *ngIf="!loginForm.get('password')?.invalid || !loginForm.get('password')?.touched" class="mb-2"></div>
                            ...
                            <p-button label="Sign In" type="submit" styleClass="w-full" [disabled]="loginForm.invalid || isLoading" [loading]="isLoading"> </p-button>

                            <div class="text-center mt-4">
                                <span class="text-muted-color">Don't have an account? </span>
                                <span class="font-medium cursor-pointer text-primary" routerLink="/auth/signup">Create one here</span>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        if (this.loginForm.valid) {
            ...
            this.authService.login(loginData).subscribe({
                next: (response) => {
                    ...
                    // Navigate to blog after successful login
                    setTimeout(() => {
